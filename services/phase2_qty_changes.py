...
from db import (
    ...
    # ✅ NEW keyed snapshot (Phase2B fix)
...
# ------------------------------------------------------------
# ✅ StarPak API touches (create/update) often set SO to Credit Held.
...
# ---------------------------------------------------------------------
# ✅ House Style Email Wrapper (used by all workflow emails)
# ---------------------------------------------------------------------
...
def detect_so4_qty_changes_or_hold(
    ...
    if new_events:
        body = "<br>".join(
            [
                f"SO {so4_sordernum} line {ln}: {old} → {new} (Item: {it})"
        ...
        <h3 style="margin:0 0 10px; font-size:15px; color:#111827;">📌 Quantity Change Details</h3>
        ...
        <p style="margin:0; font-size:14px; color:#111827;">
            ✅ <b>Next Step:</b> Please update the production estimate and reconfirm the PolyTex Job in Radius.
        </p>
        </div>
        """
        ...
        send_email(CSR_EMAILS, f"PolyTex SO {so4_sordernum} qty changed – reconfirm job required", html)
    ...
    upsert_order_state(
        ...
        last_run_id=run_id,
        last_error_summary="SO line qty changed – waiting for manual job reconfirm (PV_Req update).",
    )
    raise WorkflowHold("SO line qty changed – waiting for manual job reconfirm (PV_Req update).")
...
# ============================================================
        ...
        # --------------------------------------------------
        # 🔒 Phase 2B GUARD: only substrate material reqs
        # --------------------------------------------------
        ...
        # RequirementId is NOT stable after reconfirm — only use for logging if present
        req_id = (
            r.get("RequirementId")
        ...
        # ------------------------------------------------------------
        # ✅ Decrease/increase based on PV_Req change (printed material)
        # ------------------------------------------------------------
        is_decrease = float(requiredqty) < float(old_qty)

        # ✅ Downstream target qty ALWAYS comes from PV_Req.RequiredQty (16P4 printed material)
        ...
        # ✅ Log REQ_QTY_CHANGED once per new pvreq qty
        ...
        if not m:
            ...
            <p style="margin:0; font-size:14px; color:#111827;">
            ⚠️ The workflow detected a requirement quantity change / catch-up condition, but could not find a saved mapping between the PolyTex SO line and the downstream PO line.
            </p>

            <p style="margin:14px 0 0; font-size:14px; color:#111827;">
            ✅ <b>Action Required:</b> Please verify mapping logic or recreate the workflow so the SO → PO mapping is captured.
            </p>
            """

            html = _house_email_wrap(
                title="Workflow Issue – Missing SO → PO Mapping",
            ...
            send_email(CSR_EMAILS, f"SO {so4_sordernum} PV_Req changed – mapping missing", html)
        ...
        if pstat in PO_BLOCK_STATUSES or received > 0:
            ...
            <p style="margin:0; font-size:14px; color:#111827;">
            ⚠️ The workflow cannot update the PolyTex PO because it is already received or closed.
            </p>
            """

            html = _house_email_wrap(
                title="Update Blocked – PO Cannot Be Updated",
            ...
            send_email(CSR_EMAILS, f"SO {so4_sordernum} qty change – PO {po_num} cannot be updated", html)
        ...
        if is_decrease:
            ...
            if not _already_logged_change(sqlite_conn, int(so4_sordernum), 1, "FULFILLMENT_DECREASE_EMAIL_SENT", float(target_qty)):
                body_html = f"""
                <div style="margin:16px 0; padding:14px; border:1px solid #f3f4f6; background:#f9fafb; border-radius:10px;">
                <h3 style="margin:0 0 10px; font-size:15px; color:#111827;">📌 Quantity Decreased – Action Required</h3>
                <table style="width:100%; border-collapse:collapse; font-size:14px;">
                    <tr><td style="padding:6px 0; width:180px;"><b>PolyTex SO:</b></td><td style="padding:6px 0;">{so4_sordernum}</td></tr>
                    <tr><td style="padding:6px 0;"><b>PolyTex PO:</b></td><td style="padding:6px 0;">{po_num} (Line {po_linenum})</td></tr>
                    <tr><td style="padding:6px 0;"><b>StarPak SO:</b></td><td style="padding:6px 0;">{so_num}</td></tr>
                    <tr><td style="padding:6px 0;"><b>StarPak Job:</b></td><td style="padding:6px 0;">{job_p2 or "—"}</td></tr>
                ...
                <p style="margin:0; font-size:14px; color:#111827;">
                ⚠️ <b>Note:</b> Radius may reject decreasing the StarPak SO line qty while reserved qty exists
                (example error: “Ordered qty cannot be less than reserved quantity”).
                </p>

                <p style="margin:14px 0 0; font-size:14px; color:#111827;">
                ✅ <b>Next Step (Fulfillment):</b><br>
                1) Update the <b>StarPak Production Estimate</b> for job <b>{job_p2 or "—"}</b> to qty <b>{target_qty}</b><br>
                ...
                html = _house_email_wrap(
                    title="Action Required – StarPak Qty Decrease (Reconfirm Job)",
                    intro_html=(
                ...
                send_email(FULFILLMENT_EMAILS, f"Action Required – Qty Decrease (SO {so_num} / Job {job_p2})", html)
            ...
            # Baseline snapshot so we don’t re-trigger Phase2B on same PV_Req again
            upsert_req_snapshot_keyed(sqlite_conn, str(job_p4), reqgroup, item, requiredqty, requireddate)

            raise WorkflowHold("Phase2B: qty decreased; waiting for StarPak job reconfirm before SO/ShipReq update.")

        # ------------------------------------------------------------
        # ✅ NORMAL FLOW (increase or safe update):
        ...
        if abs(so_old_qty - float(target_qty)) <= 0.0001:
            ...
            # ✅ IMPORTANT: update_so_line_qty_api must return ok=False on real errors
        ...
        # ------------------------------------------------------------
        # ✅ INCREASE FLOW: Update ShipReq qty immediately after SO update
        # ------------------------------------------------------------
        try:
        ...
        if not _already_logged_change(sqlite_conn, int(so4_sordernum), 1, "FULFILLMENT_COMPLETE_EMAIL_SENT", float(target_qty)):
            ...
            <p style="margin:0; font-size:14px; color:#111827;">
            ✅ <b>Next Step:</b> Please reconfirm and COMPLETE this StarPak order manually in Radius.
            </p>
            """

            html = _house_email_wrap(
                title="Action Required – StarPak SO Qty Updated",
            ...
            send_email(FULFILLMENT_EMAILS, f"Action Required – Complete StarPak SO {so_num}", html)
...
def detect_starpak_reconfirm_or_complete(sqlite_conn, ro_conn, rw_conn, run_id: str, so4_sordernum: int, logger) -> bool:
    ...
    # ✅ DEBUG LOG - always print what state Phase2C sees
    logger.info(
    ...
    # ✅ Phase2C must ONLY run for StarPak HOLD states
    if step not in (
    ...
    # ✅ Derive target_qty from PO ordered qty (source-of-truth)
    target_qty = None
    try:
    ...
    # ------------------------------------------------------------
    # ✅ INCREASE HOLD STEP (MISSING IN YOUR CURRENT CODE)
    ...
    if step == "P2_SO_QTY_UPDATED_WAIT_RECONFIRM":
        ...
        # ✅ Refresh snapshot baseline ONLY on COMPLETE (not while waiting)
        try:
    ...
    if step == "P2_QTY_DECREASE_WAIT_SP_JOB_RECONFIRM":
        ...
        # ✅ Reconfirm satisfied. Now ensure SO + ShipReq match target_qty.
        ...
        # ✅ Refresh snapshot baseline ONLY on COMPLETE
        try:
            so4_lines = get_so_lines_p4(ro_conn, int(so4_sordernum))
            for ln in so4_lines:
                line_num = int(ln.get("SOrderLineNum") or ln.get("sorderlinenum") or 0)
    ...
    # ------------------------------------------------------------
    # ✅ Any other HOLD step (including manual required):
